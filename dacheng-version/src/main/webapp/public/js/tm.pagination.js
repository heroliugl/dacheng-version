...
angular.module('tm.pagination', []).directive('tmPagination',[function(){
    return {
        ...
        template: '<div class="page-list">' +
            ...
            '</ul>' +
            '<div class="page-total hidden-xs" ng-show="conf.totalItems > 0">' + source.page +
            '<input type="text" ng-model="jumpPageNum"  ng-keyup="jumpToPage($event)"/> ' + source.perpage +
            ...
            '/ ' + source.total + ' <strong>{{ conf.totalItems }}</strong> '  + source.record +
            '</div>' +
            '<div class="no-items" ng-show="conf.totalItems <= 0"><strong>' + source.nodata + '</strong></div>' +
            '</div>',
        replace: true,
        scope: {
            conf: '='
        ...
        link: function(scope, element, attrs){
            ...
            if(!scope.conf.perPageOptions){
                scope.conf.perPageOptions = [10, 15, 20, 30, 50,100];
            }
